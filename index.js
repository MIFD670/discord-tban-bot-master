...
client.on("message", message => {
  ...
  if(command === 'help') {
    ...
    title: "Greetings!",
    description: "Hey! This is a bot for trello-bans!",
    fields: [{
        name: "Prefix",
  ...
  if (command === 'tban') {
    if(!message.member.roles.some(r=>["SHR"].includes(r.name)) )
            return message.channel.send({embed: {
                color: 15406156,
                description: "You do not have permission to run this command. Error: You need the `SHR` role to run this command.",
                author: {
